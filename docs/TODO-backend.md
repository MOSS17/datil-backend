...
- **Owner notification design:** owners are *intentionally* not emailed or WhatsApped on new bookings — the goal is to keep their inboxes clean. Instead the dashboard sidebar shows an unseen-bookings badge. The `seen_at` column + `POST /appointments/{id}/seen` exist (`router.go:127`); the badge needs one more endpoint (below).

### `GET /appointments/unseen-count` — new route

- **Used by:** sidebar badge in the dashboard layout. Likely fetched on dashboard mount and on window-focus refetch (TanStack Query default).
- **Behavior:** returns the count of appointments for the authenticated user where `seen_at IS NULL` and `start_time > now() - interval '5 days'` — same filter the dashboard "latest" list uses, just COUNT instead of SELECT.
- **Success (200):** `{ count: number }`. Cap at some sentinel like 99 only if the frontend shows "99+" — otherwise return the raw count.
- **Auth:** authenticated; scoped to the JWT's `user_id` (note: `appointments` rows are scoped by `user_id`, not `business_id`).
- **Implementation:** new repository method `CountUnseenRecent(userID)` → `SELECT COUNT(*) FROM appointments WHERE user_id = $1 AND seen_at IS NULL AND start_time > now() - interval '5 days'`.
- **Index situation:** the existing `idx_appointments_user_start (user_id, start_time)` from migration 000017 already makes this query fast at MVP scale — no new index strictly required. For the optimal partial-index version, ship this migration alongside the endpoint:
  ```sql
  CREATE INDEX idx_appointments_user_unseen
